...
#define UNIQUE_RESOURCE_NOEXCEPT_NOEXCEPT_THIS_DELETER_CALL UNIQUE_RESOURCE_NOEXCEPT
#else
...
public:
  ...
  explicit unique_resource(R&& resource_, D&& deleter_, bool shouldrun = true) noexcept
      : resource(std::move(resource_)), deleter(std::move(deleter_)), execute_on_destruction{shouldrun} {}
  // move
  ...
  // resource release
  ~unique_resource() UNIQUE_RESOURCE_NOEXCEPT_NOEXCEPT_THIS_RESET { this->reset(); }
