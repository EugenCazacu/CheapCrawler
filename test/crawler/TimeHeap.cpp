...
TEST_F(TimeHeapFixture, popOnly) {
  ...
  for(int i = 0; i < 2; ++i) {
    poped.push_back(timeHeap.pop().downloadElem.url);
  }
  EXPECT_TRUE(timeHeap.empty());
...
TEST_F(TimeHeapFixture, pushAndPop) {
  ...
  EXPECT_EQ(timeHeap.pop().downloadElem.url, url1_2);
  EXPECT_TRUE(timeHeap.empty());
}
...
TEST_F(TimeHeapFixture, multiplePushAndPop) {
  ...
  timeHeap.push(QueuePopper{&emptyQueue}(dwQueueItUrl2));
  EXPECT_EQ(timeHeap.pop().downloadElem.url, url1_2);
  EXPECT_EQ(timeHeap.pop().downloadElem.url, url2_2);
  EXPECT_EQ(timeHeap.pop().downloadElem.url, url1_1);
  EXPECT_EQ(timeHeap.pop().downloadElem.url, url2_1);
  EXPECT_TRUE(timeHeap.empty());
}
